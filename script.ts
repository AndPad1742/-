...
const app = Vue.createApp({
    ...
    computed: {
        phonesOutput(){
            ...
            return this.phones.filter(phone => phone.isVisible === true);
        },
        phonesHidden(){
            console.log(`phonesHidden RUN`);
            return this.phones.filter(phone => phone.isVisible === false);
        },
        outputDiff(){
    ...
    methods: {
        checkboxEvent(){
            ...
            console.log(`checkbox.isChecked ${this.isChecked}`);
            
            let baselineName = this.phonesOutput[0].name;
            let baselineManufacturer = this.phonesOutput[0].manufacturer;
            let baselineReleaseYear = this.phonesOutput[0].releaseYear;
            let baselineScreenSize = this.phonesOutput[0].screenSize;
            let baselineManufcountry = this.phonesOutput[0].manufcountry;
            let baselineDisksize = this.phonesOutput[0].disksize;
            let baselineScreenFPS = this.phonesOutput[0].screenFPS;
            let baselineNFC = this.phonesOutput[0].nfc;
            let baselineESIMSupport = this.phonesOutput[0].esimSupport;
            let baselineWirelessCharge = this.phonesOutput[0].wirelessCharge;
            let baselinePrice = this.phonesOutput[0].price;
            
            for (let i = 1; i < this.phonesOutput.length; i++){
            ...
            console.log(`distinctName ${this.distinctName}`);
            console.log(`distinctManufacturer ${this.distinctManufacturer}`);
            console.log(`distinctReleaseYear ${this.distinctReleaseYear}`);
            console.log(`distinctScreenSize ${this.distinctScreenSize}`);
            console.log(`distinctManufcountry ${this.distinctManufcountry}`);
            console.log(`distinctDisksize ${this.distinctDisksize}`);
            console.log(`distinctScreenFPS ${this.distinctScreenFPS}`);
            console.log(`distinctNFC ${this.distinctNFC}`);
            console.log(`distinctESIMSupport ${this.distinctESIMSupport}`);
            console.log(`distinctWirelessCharge ${this.distinctWirelessCharge}`);
            console.log(`distinctPrice ${this.distinctPrice}`);
            console.log(``);
        ...
        giveOutput(quantity){
            console.log(`giveOutput(${quantity}) RUN`);
            for (let i = 0; i < quantity; i++){
                console.log(`phone[${this.phones[i].id}], isVisible = ${this.phones[i].isVisible}`);
                console.log(`output to phone[${this.phones[i].id}] granted by giveOutput`);
                this.phones[i].isVisible = true;
                console.log(`phone[${this.phones[i].id}], isVisible = ${this.phones[i].isVisible}`);
            }
            console.log(``);
        },
        removeOutput(quantity){
            console.log(`removeOutput(${quantity}) RUN`);
            for (let i = 0; i < quantity; i++){
                console.log(`phone[${this.phones[i].id}], isVisible = ${this.phones[i].isVisible}`);
                console.log(`output to phone[${this.phones[i].id}] removed by removeOutput`);
        ...
        checkOutput(){
            ...
            for (let i = 0; i < this.phones.length ; i++){
                console.log(`phone with id ${this.phones[i].id}, isVisible = ${this.phones[i].isVisible}`);
            }
            console.log(``);
        ...
        checkOutputVariables(){
            console.log(`currentlyOutputing ${this.currentlyOutputing}`);
            console.log(`outputQuantity ${this.outputQuantity}`);
            console.log(`outputDiff ${this.outputDiff}`);
            console.log(``);
        },
        countVisible(){
            this.phones.filter(phone => phone.isVisible === true);
        },
        setQuantity2(){
    ...
    created(){
        ...
        for (let i = 0; i < this.outputDiff; i++){
